...
const LIMIT_RE = /INFERENCE_CAP_ERROR|daily free limit|Error 429|rate limit|too many requests|quota exceeded|giving up after .* attempt|failed to send request|inference request failed/i;
...
const PROVIDER_ERROR_RE = /INFERENCE_CAP_ERROR|daily free limit|Error 429|rate limit|too many requests|quota exceeded|giving up after .* attempt|failed to send request|inference request failed|invalid.*key|unauthorized|expired.*key|model not found|no endpoints found/i;
...
const COOLDOWNS_FILE = process.env.TELEGRAM_COOLDOWNS_FILE ||
  path.join(__dirname, '..', `agents.cooldowns-${INSTANCE_NAME}.json`);

...
// A session that has OUTGROWN the model's context window ("Input length 273636
// exceeds the maximum allowed input length of 262112 tokens") is NOT a quota
// error — the key and model are fine; the connector's accumulated conversation
// is just longer than the window. It must be classified BEFORE LIMIT_RE (whose
// generic "inference request failed" substring would otherwise match) so a
// perfectly healthy combo is never falsely blocked for a rate-limit cooldown.
const CONTEXT_MAX_RE = /Input length \d+ exceeds the maximum allowed|maximum (context|input) length|context (length|window).*(exceeded?|too|too large)|exceeds .* (context|input) length|too (long|large).*context/i;
// The connector's cline session keeps every turn's context in one thread; over
// a long stretch of heavy edits it can outgrow even a 262k-token window (the
// observed overflow was 273,636 tokens into poolside's 262,112). Recycle the
// connector when it has been up this long AND is idle, so sessions never get
// there in the first place. 0 disables the recycle.
const MAX_SESSION_MS = Math.max(0, parseInt(process.env.TELEGRAM_MAX_SESSION_MS || '45', 10)) * 60 * 1000;
// A model is treated as exhausted for ROTATION once this many of its keys are
// on cooldown — after that many 429s on the same model, trying yet another key
// on it is futile churn (each attempt just re-hits the same daily limit and
// emits another identical notice). Records are still kept PER KEY (each combo
// earns its own block), but the scan jumps to the next model in priority order.
const SATURATED_KEYS_THRESHOLD = parseInt(process.env.TELEGRAM_SATURATED_KEYS || '2', 10) || 2;
...
const PIDS_FILE = process.env.TELEGRAM_PIDS_FILE || path.join(__dirname, '..', 'agents.pids.json');
...
const STATE_FILE = process.env.TELEGRAM_STATE_FILE || path.join(__dirname, '..', `agents.state-${INSTANCE_NAME}.json`);
...
// ── Time formatting ──────────────────────────────────────────────────────────
// User-facing timestamps use the SYSTEM's local timezone (not UTC) so the
// "frees up at HH:MM" notices match the user's wall clock. Node's
// toLocaleTimeString picks up the process TZ (inherited from the system by
// default) and abbreviates it (e.g. "BRT", "EDT", "CET").
function formatLocalTime(ms) {
  const d = new Date(ms);
  let s = d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
  const tz = /\(([^)]+)\)/.exec(d.toLocaleTimeString('en-US', { timeZoneName: 'short' }) || '');
  if (tz && tz[1]) {
    const abbr = tz[1].split(' ').pop();
    s += ' ' + (abbr.length <= 5 ? abbr : tz[1].split(' ').map(w => w[0]).join(''));
  }
  return s;
}

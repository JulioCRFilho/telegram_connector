...
async function resumeAfterRotation(chatId) {
  if (state.shuttingDown || !chatId) return;
  try {
    const before = tasks.getTaskProgress();
    if (before && before.total > 0 && before.done >= before.total) {
      log('[Resume] Task list already complete; informing user.');
    ...
    if (!before) {
      log('[Resume] No task list found in workspace; skipping auto-resume.');
      return;
    }
    ...
    const result = await withHub(async (call) => {
      ...
      return call('session.send_input', {
        sessionId,
        prompt: resumePrompt(before.file),
        mode: 'act',
        timeoutSeconds: config.RESUME_RUN_TIMEOUT_S,
    ...
    const status = finished
      ...
      : (after ? `📋 ${after.done}/${after.total} tasks completed so far.` : '');

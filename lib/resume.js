...
async function resumeAfterRotation(chatId) {
  ...
  try {
    ...
    // AUTO-CHAIN: if the resumed run itself hit a rate limit, rotate to the next
    // combo and retry automatically — no user message required. This loops until a
    // working key/model answers the request. Without this, a limit in the resumed
    // hub-session would just drop the user's request after one attempt.
    if (config.LIMIT_RE.test(err.message)) {
      ...
      const cooldownMs = rotation.parseCooldownMs(err.message) || rotation.COOLDOWN_DEFAULT_MS;
      ...
      log(`[Resume] Resumed run hit a limit — blocked key #${state.curKeyIndex} + model #${state.curModelIndex} until ${new Date(unblockAt).toISOString()}. ${allCooling ? `All combos cooling; waiting ${Math.round(next.waitMs / 60000)}m.` : `Rotating to key #${next.key} / model ${config.MODELS[next.model]}.`}`);
      chat.notifyUser(
        `🔁 Resumed run hit a rate limit — rotating to key #${next.key} / model ${config.MODELS[next.model]}${allCooling ? ` (all combos cooling for ${Math.round(next.waitMs / 60000)}m)` : ''}.`

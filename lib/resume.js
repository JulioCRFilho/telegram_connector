...
async function resumeAfterRotation(chatId) {
  ...
  try {
    ...
    if (config.PROVIDER_ERROR_RE.test(err.message)) {
      ...
      //  • LONG daily limit (>1h quoted) — the quota is model/account-wide
      //    (live logs: the same ~21h window reappears on every key of z-ai).
      //    Block the WHOLE model so we rotate to the NEXT model, and never
      //    drop the user's message: if every model is long-exhausted we park
      //    with the message queued (auto-retries at the earliest unblock).
      ...
      if (isLongWindow) {
        // Model/account-wide daily exhaustion — block every key on this model
        // so rotation naturally moves to the next model (priority order).
        const unblockAt = rotation.blockModel(state.curModelIndex, cooldownMs, 'daily limit (model-wide)', err.message.slice(0, 300));
        log(`[Resume] Daily limit on ${config.MODELS[state.curModelIndex]} — blocked ALL keys of the model until ${new Date(unblockAt).toISOString()} (model-wide).`);
        ...
        if (allCooling) {
          if (!state.shuttingDown) {
            chat.notifyUser(`⛔ All key/model groups are at their daily free limits — cooling until ~${new Date(Date.now() + next.waitMs).toISOString().slice(11, 16)} UTC. Your request is queued and retries automatically.`);
          }

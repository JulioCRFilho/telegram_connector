...
const SHORT_CYCLE_MAX_MS = 60 * 60 * 1000;
  ...
  try {
    ...
    if (config.PROVIDER_ERROR_RE.test(err.message)) {
      ...
      const cooldownMs = isRateLimit
        ...
        : (60 * 60 * 1000);
      // Short rolling window → park-and-retry, never counts toward the guard.
      const isShortCycle = isRateLimit && quotedMs > 0 && quotedMs <= SHORT_CYCLE_MAX_MS;
...
function isShortCycleOf(message) {
  ...
  return config.LIMIT_RE.test(message) && quoted > 0 && quoted <= SHORT_CYCLE_MAX_MS;
}
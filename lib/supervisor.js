...
const SYSTEM_PROMPT = (() => {
  try {
    return fs.readFileSync(path.join(__dirname, '..', 'AGENTS.md'), 'utf8').trim();
  } catch (err) {
    log(`[Rotator] Could not read AGENTS.md: ${err.message}`);
    return '';
  }
})();
...
function noticeCooldownPark(progress, waitMs) {
  chat.notifyUser(
    `⚠️ All AI providers are rate-limited right now (next free ~${formatLocalTime(Date.now() + waitMs)}).`
...
function startCline(index, modelIndex) {
  ...
  state.connectorStartedAt = startedAt;
...
function onProbeReject(key, model, cooldownMs, reason, detail, modelScoped) {
  ...
  if (allCooling) {
    ...
    chat.notifyUser(
      `⚠️ All AI providers are unavailable right now (next free ~${formatLocalTime(Date.now() + next.waitMs)}).`
      + (progress ? ` ${progress.done}/${progress.total} tasks done so far.` : '')
...
function onLimitSignal(line) {
  ...
  const quotedMs = rotation.parseCooldownMs(line);
  const cooldownMs = quotedMs || rotation.COOLDOWN_DEFAULT_MS;

  // ALWAYS block exactly the (key, model) pair that got the 429 — the API keys
  // have INDEPENDENT quotas, so one key's exhaustion says nothing about its
  // siblings (a previous "block the whole model" heuristic stamped identical
  // cooldowns across all 6 keys and idled live quota for hours). If other keys
  // on the model are also exhausted they will 429 in turn and self-block — the
  // grid converges on the truth, one combo at a time.
  const unblockAt = rotation.blockCombo(state.curKeyIndex, state.curModelIndex, cooldownMs, 'limit signal', line.slice(0, 300));
  log(`[Rotator] Blocked key #${state.curKeyIndex} + model #${state.curModelIndex} (${config.MODELS[state.curModelIndex]}) until ${new Date(unblockAt).toISOString()}${quotedMs ? ` (quoted ${Math.round(quotedMs / 60000)}m)` : ''}`);

  // Note this model produced a limit hit (informational; the grid itself now
  // drives every skip decision).
  ...
  // branch notifies through noticeCooldownPark instead, so this line only
  // fires on an actual rotation. If the next combo is the SAME model (first
  // 429 on it — not yet saturated), say "another key"; otherwise name the next
  // MODEL so a model-hop is obvious. A notice that names the SAME model as the
  // one that just failed reads as a retry-loop. Also throttled: the same
  // model→model announcement within NOTICE_DEDUPE_MS (120s) is not re-sent.
  const NOTICE_DEDUPE_MS = 120 * 1000;
  if (!allCooling) {
    state.lastLimitNotice = state.lastLimitNotice || { sig: '', at: 0 };
    const sig = `${state.curModelIndex}->${next.model}`;
    if (state.lastLimitNotice.sig !== sig || Date.now() - state.lastLimitNotice.at > NOTICE_DEDUPE_MS) {
      state.lastLimitNotice = { sig, at: Date.now() };
      const nextModelName = config.MODELS[next.model];
      const sameModel = next.model === state.curModelIndex;
      chat.notifyUser(
        sameModel
          ? `🔑 ${config.MODELS[state.curModelIndex]} hit its daily free limit — trying another key on ${nextModelName}. Your message retries automatically.`
          : `🔑 ${config.MODELS[state.curModelIndex]} hit its daily free limit — trying ${nextModelName} instead. Your message retries automatically.`
      );
    } else {
      log(`[Rotator] Skipped duplicate limit notice (${sig} within ${NOTICE_DEDUPE_MS / 1000}s window).`);
    }
  }
  ...
  // TOKEN GUARD: a resumed run's 429 arrives here ASYNCHRONOUSLY via the log —
  // the resume promise itself resolved, so resume.js's catch never sees this
  // failure and its budget check can't stop the loop. Without a counter here
  // the cycle is: resume → 429 → block combo → queueResume → resume next combo
  // → 429 … every ~18s, churning the entire grid against one dead quota. Count
  // each limit-driven re-queue against the same per-message budget the catch
  // path uses; past the cap, stop auto-retrying and hand control back to the
  // user (queued messages behind the given-up one still get promoted).
  const attempts = (state.resumeProviderRetries || 0) + 1;
  if (attempts >= resume.RESUME_MAX_PROVIDER_RETRIES) {
    log(`[Rotator] ${attempts} limit retries exhausted the resume budget for this message; not auto-retrying (token guard).`);
    const chatId = state.lastSeenChatId || config.ALLOWED_USER_ID || null;
    lastmessage.clearOne(chatId, state.lastUserMessage);   // drop ONLY the given-up message — queued ones survive
    state.lastUserMessage = null;
    state.resumeProviderRetries = 0;
    state.resumeParkRetries = 0;
    if (chatId) chat.sendTelegramMessage(chatId, `⚠️ ${config.MODELS[state.curModelIndex]} and every fallback hit their daily limits. I stopped auto-retrying to save your quota — send it again when things free up.`).catch(() => { });
    if (chat.promoteNextPending()) return;
    state.restartFromRotation = true;
    if (allCooling) {
      parkOnCooldown(next.key, next.model, next.waitMs);
    } else {
  ...
  state.resumeProviderRetries = attempts;
  state.restartFromRotation = true;
...
function onTimeoutSignal(line) {
  ...
  if ((state.resumeProviderRetries || 0) + 1 >= resume.RESUME_MAX_PROVIDER_RETRIES) {
    ...
    // A message queued BEHIND the dropped one must still be delivered: promote
    // the FIFO head first. When it returns true it has already triggered its
    // own restart — only fall back to the manual restart/park when the queue
    // was empty (or a restart was already in flight, which the guard inside
    // promoteNextPending turns into a no-op).
    if (chat.promoteNextPending()) return;
    state.restartFromRotation = true;
    if (allCooling) {
      parkOnCooldown(next.key, next.model, next.waitMs);
...
function gridWatchTick() {
  ...
  if (next.waitMs > 0) {
    ...
    // Same dedupe + wording policy as onLimitSignal: don't spam the same
    // model→model announcement within a short window.
    const NOTICE_DEDUPE_MS = 120 * 1000;
    state.lastLimitNotice = state.lastLimitNotice || { sig: '', at: 0 };
    const sig = `${state.curModelIndex}->${next.model}`;
    if (state.lastLimitNotice.sig !== sig || Date.now() - state.lastLimitNotice.at > NOTICE_DEDUPE_MS) {
      ...
      chat.notifyUser(
        sameModel
          ? `🔒 Key/model hit its daily free limit — trying another key on ${config.MODELS[next.model]}. Your messages retry automatically.`
          : `🔒 Key/model hit its daily free limit — trying ${config.MODELS[next.model]} instead. Your messages retry automatically.`
      );
    }
    state.restartFromRotation = true;
    queueResume();
...
// ── Context-overflow (session grew past the model's window) ─────────────────
// poolside rejects turns when the connector's accumulated thread exceeds the
// model's context window ("Input length 273636 exceeds the maximum allowed
// input length of 262112 tokens"). This is NOT a quota — the key and model are
// healthy; the SESSION is too long. The cure is a fresh session, so instead of
// blocking the combo for a rate-limit cooldown (which idles a perfectly good
// model for 15 min) the wrapper restarts the connector on the SAME combo and
// drives the queued message through a FRESH hub session (session.create has
// zero accumulated context), which is exactly the post-rotation resume path.
function onContextOverflow(line) {
  if (state.shuttingDown) return;
  const now = Date.now();
  if (now - state.lastContextHandledAt < LIMIT_DEDUPE_MS) {
    log('[Rotator] Duplicate context-overflow signal within dedupe window; ignoring.');
    return;
  }
  state.lastContextHandledAt = now;
  state.contextOverflowStrikes++;

  if (state.lastUserMessage) {
    lastmessage.save(state.lastSeenChatId || config.ALLOWED_USER_ID || 0, state.lastUserMessage);
  }
  log(`[Context] Session exceeded the model's context window (strike ${state.contextOverflowStrikes}) — restarting connector on a fresh session (key #${state.curKeyIndex}, model ${config.MODELS[state.curModelIndex]}). NO combo block — this is not a quota error.`);
  state.restartFromRotation = true;
  queueResume();              // the retry runs in a fresh hub session (zero history)
  scheduleRestart(state.curKeyIndex, state.curModelIndex, config.RESTART_DELAY_MS);
  chat.notifyUser('⚙️ This conversation ran long and hit the model\'s context limit — restarting it fresh. Your request is queued and retries automatically.');
}

function clearContextOverflowStrikes() {
  state.contextOverflowStrikes = 0;
}

// ── Idle session recycle ────────────────────────────────────────────────────
// The connector thread grows with every turn; over hours of heavy work it can
// reach the model's context window (the 273k overflow above). When the
// connector has been up past MAX_SESSION_MS and there is NO active turn, restart
// it on a fresh session — proactively, before the limit is ever hit. Never
// fires during an active turn / park / probe / restart, so nothing mid-task is
// cut. Disabled entirely when TELEGRAM_MAX_SESSION_MS=0.
function startIdleRecycle() {
  if (!config.MAX_SESSION_MS) return;
  const checkEvery = Math.max(60 * 1000, Math.min(config.MAX_SESSION_MS / 3, 5 * 60 * 1000));
  const iv = setInterval(() => {
    if (state.shuttingDown || state.restarting || state.startPending) return;
    if (!state.currentClinePid || !state.clineProcess) return;
    if (!state.connectorStartedAt) return;
    if (Date.now() - state.connectorStartedAt < config.MAX_SESSION_MS) return;
    if (chat.isTurnActive()) return;       // never cut an active turn
    log(`[Rotator] Connector session age limit reached (${Math.round(config.MAX_SESSION_MS / 60000)}m idle) — recycling on a fresh session to keep context small.`);
    state.restartFromRotation = true;
    scheduleRestart(state.curKeyIndex, state.curModelIndex, config.RESTART_DELAY_MS);
  }, checkEvery);
  iv.unref?.();

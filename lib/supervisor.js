...
function refreshParkRound() {
  ...
  if (p.waitMs === 0) {
    ...
    chat.notifyUser(
      `🟢 Grid refresh found key #${p.key} / model ${config.MODELS[p.model]} available early — agent is back online. Pending work resumes automatically.`
    );
...
function startParkMonitor(key, model, waitMs) {
  ...
  parkWakeTimer = setTimeout(() => {
    ...
    chat.notifyUser(
      `🟢 Cooldown over — agent is back online with key #${p.key} / model ${config.MODELS[p.model]}. Pending work resumes automatically.`
    );
...
function onLimitSignal(line) {
  ...
  // one error per key).
    const isModelScoped = /daily free limit|maximum output token|model .* (limit|exhausted|quota)/i.test(line);
  if (isModelScoped) {
    for (let k = 0; k < config.API_KEYS.length; k++) {

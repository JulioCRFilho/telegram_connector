...
function startCline(index, modelIndex) {
  ...
  state.clineProcess.on('close', (code) => {
    ...
    // invalid/exhausted key, bad model id… Probe the combo to detect the REAL
    // cooldown time (e.g., daily limit = ~22h) instead of using a generic 2-min
    // cooldown. Without this, the wrapper cycles through all combos repeatedly
    // because the 2-min cooldown expires before it cycles back.
    if (elapsedMs < rotation.CRASH_ROTATE_MS) {
      // Probe asynchronously to detect the real cooldown, then proceed with rotation
      probe.probeCombo(index, modelIndex).then((probeResult) => {
        if (probeResult.ok === false && probeResult.cooldownMs) {
          // Probe detected a definitive failure - use the real cooldown
          rotation.blockCombo(index, modelIndex, probeResult.cooldownMs);
          log(`[Rotator] Fast exit + probe confirmed limit: cooling key #${index} + model #${modelIndex} (${config.MODELS[modelIndex]}) for ${Math.round(probeResult.cooldownMs / 60000)}m (${probeResult.reason}).`);
        } else if (probeResult.ok === true) {
          // Probe succeeded - combo is actually available, was a transient crash
          log(`[Rotator] Fast exit but probe succeeded for key #${index} + model #${modelIndex} (${config.MODELS[modelIndex]}); clearing cooldown.`);
          state.blockedCombos.delete(`${index}:${modelIndex}`);
        } else {
          // Probe inconclusive or disabled - fall back to short cooldown
          rotation.blockCombo(index, modelIndex, rotation.CRASH_COOLDOWN_MS);
          log(`[Rotator] Fast exit; cooling key #${index} + model #${modelIndex} (${config.MODELS[modelIndex]}) for ${rotation.CRASH_COOLDOWN_MS / 60000}m (probe inconclusive).`);
        }
        proceedWithRotation(index, modelIndex, delay);
      }).catch((err) => {
        // Probe failed - fall back to short cooldown
        log(`[Rotator] Fast exit probe failed: ${err.message}; using default cooldown.`);
        rotation.blockCombo(index, modelIndex, rotation.CRASH_COOLDOWN_MS);
        log(`[Rotator] Fast exit; cooling key #${index} + model #${modelIndex} (${config.MODELS[modelIndex]}) for ${rotation.CRASH_COOLDOWN_MS / 60000}m.`);
        proceedWithRotation(index, modelIndex, delay);
      });
    } else {
      // Normal exit - proceed with rotation
      proceedWithRotation(index, modelIndex, delay);
    }
  });
}

// Helper function to handle rotation logic after exit handling is complete
function proceedWithRotation(index, modelIndex, delay) {
...
async function startVerified(index, modelIndex) {
  ...
  if (state.pendingRotation) {
    ...
    if (verdict && verdict.ok === false) {
      const unblockAt = rotation.blockCombo(index, modelIndex, verdict.cooldownMs || rotation.COOLDOWN_DEFAULT_MS);
  ...
  if (verdict && verdict.ok === false) {
    state.startPending = false;
    onProbeReject(index, modelIndex, verdict.cooldownMs || rotation.COOLDOWN_DEFAULT_MS, verdict.reason, verdict.detail);
    return;
  }
...
function onProbeReject(key, model, cooldownMs, reason, detail) {
  if (state.shuttingDown) return;
  const unblockAt = rotation.blockCombo(key, model, cooldownMs);
...
function onLimitSignal(line) {
  ...
  const unblockAt = rotation.blockCombo(state.curKeyIndex, state.curModelIndex, cooldownMs);
  ...
  // model rather than trying more keys on the same exhausted model. A model-
  // scoped limit ("daily free limit on model X") affects all keys.
  ...
  // Tell the user what happened. Prompt ONLY when an available key+model has
  // actually been set (requirement): when all combos are cooling down, send
  // the all-cooldown notice instead of claiming a rotation, and don't queue
  // the auto-resume — it starts once a free combo really does (see
  // scheduleRestart's afterCooldown path).
  state.restartFromRotation = true;
  const progress = tasks.getTaskProgress();
  if (allCooling) {
    noticeCooldownPark(progress, next.waitMs);
    ...
    chat.notifyUser(
      `🔁 Rate limit hit — rotating to key #${next.key} / model ${config.MODELS[next.model]}.`
      + (progress ? ` 📋 ${progress.done}/${progress.total} tasks completed.` : '')
      + ' Pending work resumes automatically.'
    );
    queueResume();

...
function onLimitSignal(line) {
  ...
  // branch notifies through noticeCooldownPark instead. NOTE: mention the next
  // MODEL (different provider), not the next key — a key-only rotation inside
  // the same model reads as a retry-loop to the user (the bug above).
  chat.notifyUser(
    `🔑 ${config.MODELS[state.curModelIndex]} hit its daily free limit — ${allCooling ? `cooling until ~${formatLocalTime(Date.now() + next.waitMs)}` : `trying ${config.MODELS[next.model]} instead`}. Your message retries automatically.`
  );

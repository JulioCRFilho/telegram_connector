...
function startCline(index, modelIndex) {
  ...
  state.clineProcess.on('close', (code) => {
    ...
    // the restart below ROTATES to the next slot. Re-arm on EVERY fast exit:
    // entries in blockedCombos are never deleted, so guarding with
    // !blockedCombos.has() meant each combo was only cooled once — after the
    // first pass every combo appeared "already blocked", was never re-cooled,
    // and the wrapper crash-looped through all keys every ~2s indefinitely
    // (the cooldown parking below could never engage).

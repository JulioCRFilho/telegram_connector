...
function startCline(index, modelIndex) {
  ...
  state.clineProcess.on('close', (code) => {
    ...
    if (elapsedMs < rotation.CRASH_ROTATE_MS) {
      const existingCooldown = state.blockedCombos.get(`${index}:${modelIndex}`) || 0;
...
function onLimitSignal(line) {
  ...
  // the all-cooldown notice instead of claiming a rotation. The auto-resume
  // is queued in BOTH branches — it must retry the last user message until
  // rotation actually sets a working key/model (see scheduleRestart's
  // afterCooldown path, which fires resumeAfterRotation on the next start).
  state.restartFromRotation = true;
  const progress = tasks.getTaskProgress();
  if (allCooling) {
    noticeCooldownPark(progress, next.waitMs);

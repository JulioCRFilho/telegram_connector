...
function onLimitSignal(line) {
  ...
  if (rotation.isLongQuotedWindow(quotedMs)) {
    // LONG daily limit (>1h quoted): the quota is model/account-wide — live
    // logs show the same ~21h window reappearing on every key of the model,
    // so rotating keys within the model is futile (and only burns retries
    // until the guard drops the user's message). Block the WHOLE model so
    // rotation moves to the NEXT model; message stays queued.
    const unblockAt = rotation.blockModel(state.curModelIndex, cooldownMs, 'daily limit (model-wide)', line.slice(0, 300));
    log(`[Rotator] Daily limit on ${config.MODELS[state.curModelIndex]} — blocked ALL keys of the model until ${new Date(unblockAt).toISOString()} (model-wide).`);
  } else {
    // SHORT rolling window (minutes) → block exactly this combo; sibling keys
    // on the same model stay live (per-key quota at this scale).
  ...
  chat.notifyUser(
    `🔑 ${config.MODELS[state.curModelIndex]} hit its daily free limit — ${allCooling ? `cooling until ~${new Date(Date.now() + next.waitMs).toISOString().slice(11, 16)} UTC` : `trying ${config.MODELS[next.model]} instead`}. Your message retries automatically.`
  );

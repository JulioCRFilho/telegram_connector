...
function pauseCurrentForNewMessage(chatId, newText) {
  ...
  const queued = interrupted.push(chatId, current);
  log(`[Interrupt] Pausing current task (${queued} paused): "${String(current).slice(0, 80)}"`);
  sendTelegramMessage(chatId,
    `⏸ Pausing the current task (saved — nothing is lost) to handle your new message first.`
    + (queued > 1 ? ` ${queued - 1} earlier paused task(s) still queued.` : '')
    + `\nI'll resume it automatically right after.`);
...
function resumeNextInterrupted() {
  ...
  if (!rec) return false;
  log(`[Interrupt] Resuming paused task: "${String(rec.text).slice(0, 80)}"`);
  state.lastUserMessage = rec.text;
  state.resumeAttempts = 0;
  lastmessage.save(rec.chatId, rec.text);
  sendTelegramMessage(rec.chatId,
    `▶️ Done with the newer request — resuming the paused task now:\n"${String(rec.text).slice(0, 300)}"`);
...
function onTurnDone(ok) {
  ...
  stopTurn();
  // The newer request has been answered — continue the oldest paused task, if
  // any. (A failed turn keeps the just-failed message pending via the normal
  // resume machinery; piling the paused task on top would muddle the retry.)
  if (ok) resumeNextInterrupted();
}

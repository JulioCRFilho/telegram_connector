...
async function ackQueuedDuringPark(earliestUnblockAt) {
  ...
  try {
    ...
    const when = formatLocalTime(earliestUnblockAt);
    for (const u of body.result || []) {
      ...
      sendTelegramMessage(chatId, `Got it — you're queued behind the current rate limit (frees up ~${when}). I'll start on this automatically and send the result here.`);
    }
  } finally {
...
function startTurn(chatId) {
  ...
  sendTelegramMessage(chatId, allBlocked
    ? `⚠️ The AI provider is rate-limited right now (frees up ~${formatLocalTime(rotation.earliestUnblock())}). Your request is queued — I'll start automatically as soon as capacity is back.`
    : turnAckText());
}
...
function statusText() {
  ...
  return [
    ...
    rec.waitMs > 0
      ? `⏳ Next free ~${formatLocalTime(Date.now() + rec.waitMs)}`

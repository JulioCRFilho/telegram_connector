...
async function ackQueuedDuringPark(earliestUnblockAt) {
  ...
  let body;
  try {
    const res = await fetch(`https://api.telegram.org/bot${config.TELEGRAM_BOT_TOKEN}/getUpdates?timeout=0&offset=${parkUpdateOffset}`);
...
function onTurnStarted(line) {
  ...
  if (preview.startsWith('/')) {
    // Wrapper-level consultation command: /keys (or /keymap, /cooldowns)
    // replies with the persisted cooldown grid — which (key, model) pairs are
    // on a daily-limit cooldown, their REAL remaining time, and the recommended
    // combo rotation will use next. Any reply the connector itself makes to the
    // unknown command is harmless — the grid is the authoritative answer.

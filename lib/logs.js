...
function pollLogs() {
  tailLog(config.SHARED_CLINE_LOG, (line) => {
    ...
    // Context-overflow MUST classify before LIMIT_RE: the generic
    // "inference request failed" substring (which LIMIT_RE matches) rides along
    // inside the same error, and a healthy combo would otherwise be blocked
    // for a rate-limit cooldown when the real problem is a session that
    // outgrew the model's window.
    if (config.CONTEXT_MAX_RE.test(line)) {
      chat.onTurnDone(false);
      procs.killIfStaleConnector(line);
      supervisor.onContextOverflow(line);
      return;
    }
    if (config.LIMIT_RE.test(line)) {
      ...
      chat.onTurnDone(false);
      // If the limit came from a stale connector (not our pid), kill it so it
      // stops stealing getUpdates and hitting the same exhausted combo.
      procs.killIfStaleConnector(line);
      supervisor.onLimitSignal(line);
      return;
    }
    chat.handleTurnEvent(line);
    // A completed reply proves the current combo works — reset timeout
    // strikes so a future isolated timeout starts from scratch (done here
    // rather than in chat.js to avoid a chat→supervisor circular import).
    if (/"msg":"Telegram reply completed"/.test(line)) {
      supervisor.clearTimeoutStrikes();
      supervisor.clearContextOverflowStrikes();
    }
    // Timeout branch AFTER the limit branch: "The operation timed out." from
    // the bridge is a provider/hub timeout, not a quota error. Without this,
    // the wrapper stayed on the same timing-out combo forever (turn failed,
    // no rotation, message silently dropped).
    if (config.TIMEOUT_RE.test(line)) {
      chat.onTurnDone(false);              // stop progress pings for the dead turn
      supervisor.onTimeoutSignal(line);
    }
  });

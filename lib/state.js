...
module.exports = {
  ...
  // Models that have hit a rate limit. When a model is in this set, nextCombo
  // prefers to rotate to a DIFFERENT model rather than trying more keys on the
  // same exhausted model — a model-scoped limit ("daily free limit on model X")
  // affects all keys, so rotating keys within the same model is futile.

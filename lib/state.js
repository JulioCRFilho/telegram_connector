...
module.exports = {
  ...
  // Throttle for the "hit its daily free limit — trying X instead" chat
  // notices: last announcement signature `curModel->nextModel` + wall-clock ms.
  lastLimitNotice: { sig: '', at: 0 },
  ...
  // Context-overflow handling (session grew past the model's window): the
  // error is NOT a quota — a fresh connector/hub session with zero accumulated
  // context resolves it. Strike count is informational/log + escalation; the
  // 5s dedupe mirrors lastLimitHandledAt (the bridge logs a failing turn twice).
  contextOverflowStrikes: 0,
  lastContextHandledAt: 0,
  // Epoch the current connector process was spawned (for the idle-session
  // recycle that stops sessions from ever reaching the context limit).
  connectorStartedAt: 0,
};
...
async function probeHttp(keyIndex, modelIndex) {
  ...
  if (body && body.error) {
    ...
    if (v.blocked) {
      ...
      // Informational only: the limit text names the model, but the quota is
      // PER KEY — the caller blocks just this (key, model) combo.
      const isModelScoped = /daily free limit|model .* (limit|exhausted|quota)/i.test(v.detail || '');
      return { ok: false, cooldownMs: v.cooldownMs, reason: v.reason, detail: v.detail, modelScoped: isModelScoped };
    }
...
async function probeCombo(keyIndex, modelIndex) {
  ...
  // If the cooldown grid says this combo is blocked, don't waste time probing —
  // the spawn-based probe only tests Telegram connectivity, NOT API
  // availability, so it would pass even for a combo that's on cooldown.
  const unblockAt = rotation.comboUnblockAt(keyIndex, modelIndex);
  if (unblockAt > Date.now()) {
    log(`[Probe] key #${keyIndex} + model #${modelIndex} (${model}) — skipped (on cooldown until ${new Date(unblockAt).toISOString()}).`);
    return { ok: false, cooldownMs: unblockAt - Date.now(), reason: 'on cooldown (grid)', detail: `Combo blocked until ${new Date(unblockAt).toISOString()}` };
  }
  ...
  return new Promise((resolve) => {
    ...
    // After "connected as" we DON'T resolve immediately — the connector can start
    // fine even when the key/model is rate-limited (the Telegram connection works,
    // but inference calls fail). We wait a settling window so the connector has time
    // to attempt an inference call and surface a 429. Only if no error appears do we
    // declare the combo valid.
    let connectedAt = 0;
    ...
    child.stdout.on('data', (buf) => {
      ...
      // Check for successful connection — start the settle window, don't resolve yet
      if (!connectedAt && /Telegram connector ready|connected as/.test(text)) {
        connectedAt = Date.now();
        clearSettle();
        settleTimer = setTimeout(() => {
          if (!resolved) resolveOnce({ ok: true, reason: 'ok' });
        }, SETTLE_WINDOW_MS);
      }
    });
    ...
    child.stderr.on('data', (buf) => {
      ...
      // Check for error messages that indicate the key/model is unavailable
            if (/INFERENCE_CAP_ERROR|daily free limit|Error 429|rate limit|too many requests|quota exceeded|maximum output token limit|invalid.*key|unauthorized|expired.*key|model not found|no endpoints found/i.test(text)) {
        clearSettle();
    ...
    child.on('close', (code) => {
      ...
      if (!resolved) {
        ...
        const output = stdout + stderr;
                if (/INFERENCE_CAP_ERROR|daily free limit|Error 429|rate limit|too many requests|quota exceeded|maximum output token limit/i.test(output)) {
          const cooldownMs = rotation.parseCooldownMs(output) || rotation.COOLDOWN_DEFAULT_MS;
          // Informational only: the limit text names the model, but the quota
          // is PER KEY — the caller blocks just this (key, model) combo.
                    const isModelScoped = /daily free limit|maximum output token|model .* (limit|exhausted|quota)/i.test(output);
          resolveOnce({ ok: false, cooldownMs, reason: 'provider error', detail: output.slice(0, 200), modelScoped: isModelScoped });

...
// Pre-flight availability probe — enabled by default for ALL providers.
// The probe tests each (key, model) combo against the real API BEFORE launching
// the connector. This prevents starting a connector that will immediately crash
// due to rate limits, invalid keys, or unavailable models.
//
// Force it explicitly with TELEGRAM_PROBE_ENABLED=1/0 if needed.
const PROBE_ENV = String(process.env.TELEGRAM_PROBE_ENABLED || '').toLowerCase();
const PROBE_ENABLED = PROBE_ENV === ''
  ? true  // Enabled by default for all providers

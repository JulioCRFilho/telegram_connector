...
// Zeroes the WHOLE grid: every key×model cooldown record is dropped (memory +
// persisted file) and the in-memory model-limit marks are cleared. Used by the
// /reset chat command — the explicit "provider says I have capacity again,
// stop believing the old 429s" escape hatch. Cancels any debounced save first
// so a stale write can't resurrect the cleared grid. Returns how many records
// were cleared (for the user-facing confirmation).
function resetAll() {
  const cleared = state.blockedCombos.size;
  if (saveTimer) { clearTimeout(saveTimer); saveTimer = null; }
  state.blockedCombos.clear();
  if (state.modelLimitHit && state.modelLimitHit.clear) state.modelLimitHit.clear();
  try {
    fs.writeFileSync(config.COOLDOWNS_FILE, '{}\n');
  } catch (err) {
    log(`[Cooldown] Couldn't persist cleared grid: ${err.message}`);
  }
  log(`[Cooldown] Grid RESET — ${cleared} record(s) cleared, all combos marked available.`);
  return cleared;
}

module.exports = { load, save, scheduleSave, resetAll, MAX_RECORD_AGE_MS, gridKeyValid, totalCombos, isComboFree };
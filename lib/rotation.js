...
// window reappearing on keys #0..#5 of z-ai), so rotating keys within the
// model is futile and only burns retries until the message gets dropped.
// Long-window 429s must block the WHOLE model so rotation moves to the next
// model; if every model is long-exhausted we park (message stays queued).
...
// Blocks EVERY key of a model until now+cooldownMs (each key×model record —
// the grid keeps all 18 slots, so "rounds" stay consistent). Returns the
// latest unblock epoch across the model. Used only for LONG-window daily
// limits where live evidence shows the quota is model/account-wide.
function blockModel(modelIdx, cooldownMs, reason, detail) {
  let latest = 0;
  for (let k = 0; k < config.API_KEYS.length; k++) {
    const at = blockCombo(k, modelIdx, cooldownMs, reason, detail);
...
function scanFromCurrent() {
  ...
  const modelOrder = modelsByPriority();

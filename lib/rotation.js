...
//
// A model-scoped rate limit blocks ALL keys of that model in the grid, so the
// priority scan naturally skips it; state.modelLimitHit (key-scoped limits)
// additionally demotes the current model to a second pass — a key-scoped
// limit leaves its siblings usable, but the priority models come first anyway.
function scanFromCurrent() {
  ...
  for (const m of modelOrder) {
    if (m === state.curModelIndex && state.modelLimitHit.has(m)) continue;
    for (let step = 0; step < nKeys; step++) {
      const k = (startKey + step) % nKeys;
      if (comboUnblockAt(k, m) <= now) return [k, m];
    }
  }
  // Second pass including the limit-hit current model (grid-blocked keys are
  // still skipped by comboUnblockAt — only genuinely free slots can win).
  for (const m of modelOrder) {
    for (let step = 0; step < nKeys; step++) {
      const k = (startKey + step) % nKeys;
      if (comboUnblockAt(k, m) <= now) return [k, m];

...
// Stashes a paused task. `dossier` is the task's progress narration captured
// at pause time — the resume prompt needs it, because a fresh hub session has
// NO other memory of what the interrupted run had already done. Returns the
// number of paused tasks now queued.
function push(chatId, text, dossier) {
  const list = loadAll();
  ...
  list.push({
    chatId,
    text,
    dossier: Array.isArray(dossier) ? dossier.slice(-20) : [],
    updatedAt: Date.now(),
  });

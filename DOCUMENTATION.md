...
| Provider rate-limited | `⚠️ The AI provider is rate-limited right now (frees up ~HH:MM (your local time)). Your request is queued — I'll start automatically as soon as capacity is back.` |
...
| Everything cooling | `⚠️ All AI providers are unavailable right now (next free ~HH:MM (your local time)). I'll keep checking automatically…` |
...
| Message during park | `⏳ … queued until ~HH:MM (your local time)` (persisted, retried automatically) |
...
- `ackQueuedDuringPark(earliestUnblockAt)` — **park-time poller**: while every
  ...
  until ~HH:MM (your local time)" notice and persists it (`lastmessage.save`) so the
